...
function MobContent({ step, inp }) {
  ...
  if (step===0) return <>
    <Field label="Agent Name" required hint="As per your government-issued ID"><input className={inp} placeholder="Enter your full name"/></Field>
    <Field label="Registration/License No" hint="If applicable"><input className={inp} placeholder="Enter registration or license number"/></Field>
    <Field label="Property Type" required><select className={inp} defaultValue=""><option value="" disabled>Select Property Type</option><option>Residential</option><option>Commercial</option><option>Industrial</option></select></Field>
    <Field label="Office Address" required><textarea className={`${ta} min-h-[65px]`} placeholder="Enter your agency office address (Street, Area, City, State, PIN)"/></Field>
    <Field label="Contact Person Name" required><input className={inp} placeholder="Enter contact person name"/></Field>
    <Field label="Phone Number" required><input className={inp} type="tel" placeholder="Enter your 10-digit mobile number"/></Field>
    ...
    <Field label="GST/PAN No" hint="If applicable"><input className={inp} placeholder="Enter GST or PAN number"/></Field>
  </>;
  if (step===1) return <>
    ...
    <Field label="Property Owner/Building Name" required><input className={inp} placeholder="Enter property owner or building name"/></Field>
    <Field label="Property Type" required>{["Residential","Commercial","PG/Hostel","Mixed Use"].map(t=><label key={t} className="flex items-center gap-2 text-[10px] mb-1"><input type="radio" name="ptype" className="accent-[#00695C] w-3 h-3"/>{t}</label>)}</Field>
    <Field label="Authorization Status" required><div className="flex gap-4"><label className="flex items-center gap-1.5 text-[10px]"><input type="radio" name="authStatus" className="accent-[#00695C] w-3 h-3"/>Pending</label><label className="flex items-center gap-1.5 text-[10px]"><input type="radio" name="authStatus" className="accent-[#00695C] w-3 h-3"/>Verified</label></div></Field>
    <Field label="Agreement/Authorization Letter" required>
      <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
        <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="m-auth-letter"/>
  ...
  if (step===2) return <>
    ...
    <Field label="Property/Project Name" required><input className={inp} placeholder="e.g. Green Valley Apartment"/></Field>
    <Field label="Property Address/Location" required><textarea className={`${ta} min-h-[55px]`} placeholder="Enter complete property address"/></Field>
    <div className="grid grid-cols-2 gap-1.5"><Field label="City" required><input className={inp} placeholder="Enter city name"/></Field><Field label="Pincode" required><input className={inp} placeholder="Enter pincode"/></Field></div>
    <Field label="Total Area (sq/acre)" required><input className={inp} placeholder="Enter total area"/></Field>
    <Field label="Unit/Room Details (if applicable)"><input className={inp} placeholder="e.g. Unit no., Floor, etc."/></Field>
    <Field label="Configuration" required><div className="grid grid-cols-2 gap-1.5">{["1 BHK","2 BHK","3 BHK","4 BHK","Other"].map(c=><label key={c} className="flex items-center gap-2 text-[10px]"><input type="radio" name="config" className="accent-[#00695C] w-3 h-3"/>{c}</label>)}</div></Field>
    <Field label="Furnishing Status" required>{["Full Furnished","Semi Furnished","Unfurnished"].map(f=><label key={f} className="flex items-center gap-2 text-[10px] mb-1"><input type="radio" name="furnish" className="accent-[#00695C] w-3 h-3"/>{f}</label>)}</Field>
    <Field label="Project Status" required><div className="flex gap-4"><label className="flex items-center gap-1.5 text-[10px]"><input type="radio" name="projectStatus" className="accent-[#00695C] w-3 h-3"/>Ready to Move</label><label className="flex items-center gap-1.5 text-[10px]"><input type="radio" name="projectStatus" className="accent-[#00695C] w-3 h-3"/>Under Construction</label></div></Field>
  </>;
  if (step===3) return <>
    ...
    <Field label="Expected Price (Sale/Rent)" required><input className={inp} placeholder="e.g. 50,00,000 or 20,000/month"/></Field>
    <Field label="Price Type" required><div className="flex gap-4"><label className="flex items-center gap-1.5 text-[10px]"><input type="radio" name="priceType" className="accent-[#00695C] w-3 h-3"/>Fixed</label><label className="flex items-center gap-1.5 text-[10px]"><input type="radio" name="priceType" className="accent-[#00695C] w-3 h-3"/>Negotiable</label></div></Field>
    <Field label="Commission" required><input className={inp} placeholder="Enter commission amount or percentage"/></Field>
    <Field label="Security Deposit (if rental)"><input className={inp} placeholder="Enter security deposit amount"/></Field>
    <Field label="Amenities"><div className="flex flex-wrap gap-1 mt-0.5">{["Lift","Power Backup","Security","Water Supply","Park","Gym","Swimming Pool","Parking","Children Play Area","Other"].map(a=><span key={a} className="px-1.5 py-0.5 text-[9px] bg-teal-50 text-[#00695C] rounded-full border border-teal-200">{a}</span>)}</div></Field>
  </>;
  if (step===4) return <>
    ...
    {[{id:"m-agent-license",e:"📜",t:"Upload Agent License/Registration Proof",s:"Upload agent license or registration document",l:"Agent License/Registration Proof",r:true},{id:"m-owner-auth",e:"📄",t:"Upload Authorization/Agreement from Owner/Builder",s:"Upload signed authorization agreement",l:"Authorization/Agreement from Owner/Builder",r:true}].map(d=>(
...
function DtContent({ step, inp }) {
  ...
  if (step===0) return <>
    <FieldDt label="Agent Name" required hint="As per your government-issued ID"><input className={inp} placeholder="Enter your full name"/></FieldDt>
    <FieldDt label="Registration/License No" hint="If applicable"><input className={inp} placeholder="Enter registration or license number"/></FieldDt>
    <FieldDt label="Property Type" required><select className={inp} defaultValue=""><option value="" disabled>Select Property Type</option><option>Residential</option><option>Commercial</option><option>Industrial</option></select></FieldDt>
    <FieldDt label="Office Address" required><textarea className={`${ta} min-h-[70px]`} placeholder="Enter your agency office address (Street, Area, City, State, PIN)"/></FieldDt>
    <FieldDt label="Contact Person Name" required><input className={inp} placeholder="Enter contact person name"/></FieldDt>
    <FieldDt label="Phone Number" required><input className={inp} type="tel" placeholder="Enter your 10-digit mobile number"/></FieldDt>
    ...
    <FieldDt label="GST/PAN No" hint="If applicable"><input className={inp} placeholder="Enter GST or PAN number"/></FieldDt>
  </>;
  if (step===1) return <>
    ...
    <FieldDt label="Property Owner/Building Name" required><input className={inp} placeholder="Enter property owner or building name"/></FieldDt>
    <FieldDt label="Property Type" required>{["Residential","Commercial","PG/Hostel","Mixed Use"].map(t=><label key={t} className="flex items-center gap-2 text-xs sm:text-sm mb-2"><input type="radio" name="ptype" className="accent-[#00695C] w-3.5 h-3.5"/>{t}</label>)}</FieldDt>
    <FieldDt label="Authorization Status" required><div className="flex gap-5"><label className="flex items-center gap-2 text-xs sm:text-sm"><input type="radio" name="authStatus" className="accent-[#00695C] w-3.5 h-3.5"/>Pending</label><label className="flex items-center gap-2 text-xs sm:text-sm"><input type="radio" name="authStatus" className="accent-[#00695C] w-3.5 h-3.5"/>Verified</label></div></FieldDt>
    <FieldDt label="Agreement/Authorization Letter" required>
      <div className="upload-box cursor-pointer hover:bg-green-50"><input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="auth-letter-dt"/>
  ...
  if (step===2) return <>
    ...
    <FieldDt label="Property/Project Name" required><input className={inp} placeholder="e.g. Green Valley Apartment"/></FieldDt>
    <FieldDt label="Property Address/Location" required><textarea className={`${ta} min-h-[70px]`} placeholder="Enter complete property address (Street, Area, Locality)"/></FieldDt>
    <div className="grid grid-cols-2 gap-2"><FieldDt label="City" required><input className={inp} placeholder="Enter city name"/></FieldDt><FieldDt label="Pincode" required><input className={inp} placeholder="Enter pincode"/></FieldDt></div>
    <FieldDt label="Total Area (sq/acre)" required><input className={inp} placeholder="Enter total area"/></FieldDt>
    <FieldDt label="Unit/Room Details (if applicable)"><input className={inp} placeholder="e.g. Unit no., Floor, etc."/></FieldDt>
    <FieldDt label="Configuration" required><div className="flex flex-wrap gap-3">{["1 BHK","2 BHK","3 BHK","4 BHK","Other"].map(c=><label key={c} className="flex items-center gap-2 text-xs sm:text-sm"><input type="radio" name="config" className="accent-[#00695C] w-3.5 h-3.5"/>{c}</label>)}</div></FieldDt>
    <FieldDt label="Furnishing Status" required>{["Full Furnished","Semi Furnished","Unfurnished"].map(f=><label key={f} className="flex items-center gap-2 text-xs sm:text-sm mb-2"><input type="radio" name="furnish" className="accent-[#00695C] w-3.5 h-3.5"/>{f}</label>)}</FieldDt>
    <FieldDt label="Project Status" required><div className="flex gap-5"><label className="flex items-center gap-2 text-xs sm:text-sm"><input type="radio" name="projectStatus" className="accent-[#00695C] w-3.5 h-3.5"/>Ready to Move</label><label className="flex items-center gap-2 text-xs sm:text-sm"><input type="radio" name="projectStatus" className="accent-[#00695C] w-3.5 h-3.5"/>Under Construction</label></div></FieldDt>
  </>;
  if (step===3) return <>
    ...
    <FieldDt label="Expected Price (Sale/Rent)" required><input className={inp} placeholder="e.g. 50,00,000 or 20,000/month"/></FieldDt>
    <FieldDt label="Price Type" required><div className="flex gap-5"><label className="flex items-center gap-2 text-xs sm:text-sm"><input type="radio" name="priceType" className="accent-[#00695C] w-3.5 h-3.5"/>Fixed</label><label className="flex items-center gap-2 text-xs sm:text-sm"><input type="radio" name="priceType" className="accent-[#00695C] w-3.5 h-3.5"/>Negotiable</label></div></FieldDt>
    <FieldDt label="Commission" required><input className={inp} placeholder="Enter commission amount or percentage"/></FieldDt>
    <FieldDt label="Security Deposit (if rental)"><input className={inp} placeholder="Enter security deposit amount"/></FieldDt>
    <FieldDt label="Amenities"><div className="flex flex-wrap gap-1.5 mt-1">{["Lift","Power Backup","Security","Water Supply","Park","Gym","Swimming Pool","Parking","Children Play Area","Other"].map(a=><span key={a} className="chip text-xs sm:text-sm">{a}</span>)}</div></FieldDt>
  </>;
  if (step===4) return <>
    ...
    {[{id:"agent-license-dt",e:"📜",t:"Upload Agent License/Registration Proof",s:"Upload agent license or registration document",l:"Agent License/Registration Proof",r:true},{id:"owner-auth-dt",e:"📄",t:"Upload Authorization/Agreement from Owner/Builder",s:"Upload signed authorization agreement",l:"Authorization/Agreement from Owner/Builder",r:true}].map(d=>(
