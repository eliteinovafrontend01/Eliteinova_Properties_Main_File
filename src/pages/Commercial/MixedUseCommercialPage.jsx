// MixedUseCommercialPage.jsx
import React, { useState, useEffect } from "react";
import { ChevronDown, Search, Home, MapPin, Star, Filter, X, Building, Landmark, Warehouse, Building2, Briefcase, Users, Wifi, Coffee, Car, Clock, Shield, Layers, Store, Utensils, Hotel, Stethoscope, GraduationCap, Cpu, CarFront, Factory, Wrench, Trees } from "lucide-react";
import { useNavigate, useLocation } from "react-router-dom";
import backgroundImage from "../../assets/house1.png";
import MixedUseCommercialFilter from "../../components/filters/Commercial/MixedUseCommercialFilter";
import MixedUseCommercial from "../../components/propertycard/Commercial/MixedUseCommercial";

const MixedUseCommercialPage = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const [activeButton, setActiveButton] = useState("Rent");
  const [activeMixedUseType, setActiveMixedUseType] = useState("Mixed-use Commercial Property");
  ...
  useEffect(() => {
    ...
    const activeType = mixedUseTypes.find(type => type.path === currentPath);
    if (activeType) {
      setActiveMixedUseType(activeType.name);
    }
  }, [location.pathname]);

  const handleNavigation = (path, typeName = null) => {
    if (typeName) {
      setActiveMixedUseType(typeName);
    }
    navigate(path);
  };
        ...
        <section className="w-full h-[200px] md:h-[280px] relative flex items-center justify-center overflow-hidden group pb-6">
          ...
          <div className="max-w-none mx-auto px-6 relative z-10 text-center w-full flex flex-col items-center justify-center gap-2">
            <div className="mb-2 inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r from-teal-600/20 to-emerald-600/20 backdrop-blur-lg border border-teal-300/20 animate-float-glow shadow-[0_0_30px_rgba(0,105,92,0.3)]">
              <Layers className="w-4 h-4 text-teal-300 animate-spin-slow" />
            ...
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-2 animate-slide-up drop-shadow-[0_0_30px_rgba(0,105,92,0.5)]">
              Find Your Perfect <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-300 via-emerald-300 to-teal-300 animate-gradient-text">Mixed-Use Property</span>
            </h1>
            
            <p className="text-base md:text-lg text-white/90 mb-3 max-w-3xl mx-auto leading-relaxed">
              Discover versatile mixed-use properties combining retail, office, and residential spaces in prime locations
            </p>
            ...
            <div className="hidden md:block space-y-4">
              <div className="flex gap-4 items-center">
                ...
                <div className="relative flex-1 group">
                  ...
                  <input
                    type="text"
                    placeholder="Search mixed-use properties by city, locality, or business district"
              ...
              <div className="flex flex-wrap gap-1.5 max-h-auto overflow-y-auto scrollbar-thin w-full">
                {mixedUseTypes.map((type) => {
                  const isActive = activeMixedUseType === type.name;
                  return (
                    <button
                      key={type.name}
                    ...
                    >
                      ...
                      <span className="relative z-10 flex items-center gap-2">
                        <Layers className={`w-3.5 h-3.5 transition-transform duration-300 ${isActive ? 'rotate-12' : 'group-hover:rotate-12'}`} />
                        {type.name}
                      </span>
                    </button>
                  );
...
export default MixedUseCommercialPage;